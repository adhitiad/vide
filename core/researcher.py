...
# Nonaktifkan peringatan SSL Request Tidak Aman (karena penggunaan verify=False)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)


def _get_google_news_fallback() -> str:
    """Fallback ke fungsi RSS Google News ID lama."""
    url = "https://news.google.com/rss/search?q=kripto+OR+bitcoin+OR+saham+global+OR+artificial+intelligence+OR+teknologi+OR+hot+OR+sensasi+OR+viral+OR+trendingwhen:1d&hl=id&gl=ID&ceid=ID:id"
...
def get_trending_topic() -> str:
    ...
    url = f"https://www.reddit.com/r/{chosen_sub}/top.json?limit=100&t=day"
    ...
    try:
        # Tambahkan verify=False untuk menembus filter Injeksi Sertifikat/DNS ISP yang sering memblokir Reddit
        ...
        if parsed_posts:
            ...
            logger.info(
                f"💡 Reddit Predictor menemukan topik panas: '{chosen_topic}' ({parsed_posts[0][1]} Komentar)"
            )
            return chosen_topic
        else:
            logger.warning(
                "⚠️ Tidak ada tren Reddit yang cocok dengan kata kunci target."
            )
        ...
        err_msg = str(e).lower()
        if (
            "internet-positif" in err_msg
            or "internetpositif" in err_msg
            or "forbidden" in err_msg
        ):
            logger.warning(
                "⚠️ Reddit diblokir ISP (Internet Positif). Aman, otomatis beralih ke Google News..."
            )
        else:
            logger.warning(
                f"⚠️ Gagal mengambil data Reddit: {e}. Beralih ke Google News..."
            )
        return _get_google_news_fallback()
    except Exception as e:
        logger.error(f"❌ Kesalahan pemrosesan tren: {e}")
        return _get_google_news_fallback()
